		...
		<div class="col-md-3 col-xs-12">
			<div class="box box-primary">
				<div class="box-body">
					...
					<a class="btn btn-primary btn-block" id="btn-edit" style="display: none;"
		...
		<div class="nav-tabs-custom">
			...
			<div class="tab-content">
				<div class="tab-pane active" id="tab_1">
					<div class="row">
						<div class="col-md-12">
							<a id="btnAddCert" style="cursor:pointer;display: none;margin-right: 10px;"
								class="pull-right"><i class="fa fa-plus"></i> &nbspCertification</a>
						</div>
					</div>
				...
				<div class="tab-pane" id="tab_2">
					<div class="box box-solid">
						<div class="box-header with-border">
							...
							<a id="btnAddCertPartner" class="pull-right" style="cursor:pointer;display: none;"><i
									class="fa fa-plus"></i>&nbspCertificate Partner</a>
						</div>
				...
				<div class="tab-pane" id="tab_3">
					<div class="row">
						<div class="col-md-12">
							<a onclick="btnTarget('{{$data->id_partnership}}')" id="btnAddTarget"
								style="display: none;cursor:pointer;float: right;margin-right: 10px;"><i
									class="fa fa-plus"></i> &nbspTarget</a>
						</div>
					</div>
					<div class="row">
						<div class="col-md-12">
...
<script type="text/javascript">
	...
	function addListCert() {
		...
		append = append +
			"		<div class='checkbox'><label><input id='cbLifetimeAddCert' class='cbLifetimeAddCert' type='checkbox'>Check for Lifetime Date</label></div>"
		append = append + " </td>"
		append = append + " <td>"
		append = append + "		<input data-value='" + i +
		...
		append = append + " <select class='form-control select2-person' data-value='" + i +
			"' id='select2-person' style='width: 100%!important' name='cert_person[]'></select> "
		append = append + " </td>"
			...
			success: function (result) {
				...
				$.each(result.data.cert_user, function (key, value) {
					...
					$.each(value, function (key, data) {
						...
						append = append + '<h3 class="box-title">' + data.name_certification +
							'</h3>'
						append = append + '</div>'
						...
						append = append + '<button value="' + data.id +
							'" class="btn btn-xs btn-danger pull-right margin-left btn-delete-eng" style="vertical-align: top; width: 60px;display:none"><i class="fa fa-trash"></i> Delete</button>'
						if (data.certificate != null) {
							append = append +
								'<a target="_blank" href="{{url("image/certificate_engineer/")}}/' +
								data.certificate +
								'"><button class="btn btn-xs btn-info pull-left"><i class="fa fa-download"></i> Download</button></a>'
						} else {
							append = append +
								'<button class="btn btn-xs btn-default pull-left disabled" title="Certificate is Empty!"><i class="fa fa-download"></i> Download</button>'
						}
						append = append +
							'<button class="btn btn-xs btn-primary pull-right btn-edit-eng" value="' +
							data.id + "," + data.name + "," + data.name_certification + "," +
							data.expired_date +
							'" name="edit_hurec" style="vertical-align: top; width: 60px;margin-right:10px;display:none"><i class="fa fa-pencil"></i> Edit</button>'
						append = append + '</div>'
						append = append + '</div>'
						append = append + "</div>"
					})
				...
				$(".btn-edit-eng").click(function () {
					...
					appendModal = appendModal +
						"<div class='checkbox'><label><input id='cbLifetime' type='checkbox'>Check for Lifetime Date</label></div>"
					appendModal = appendModal + " 	</td>"
					appendModal = appendModal + " 	<td>"
				...
				$(".btn-dropdown-toggle").click(function () {
					if ($(this).closest("div").closest(".box-header").next().is(":visible")) {
						...
						$(this).find("i").removeClass('fa fa-chevron-down').addClass(
							'fa fa-chevron-up')
					} else {
						$(this).closest("div").closest(".box-header").next().show()
						$(this).find("i").removeClass('fa fa-chevron-up').addClass(
							'fa fa-chevron-down')
					}
				})
			}
		...
		$.ajax({
			...
			success: function (result) {
				...
				$.each(result.data, function (key, value) {
					...
					appendList = appendList + '	<input type="checkbox" class="checked-' + key +
						'" data-value="' + value.id + '">'
					appendList = appendList + '	<span class="text" id="textList" data-value=' + key +
						'>' + value.target + ' - ' + value.countable + '</span>'
					appendList = appendList + ' <small class="label label-warning status-' + key +
						'">' + value.status + '</small>'
					appendList = appendList + '	<div id="targetTools" class="tools activeTrash-' +
						key + '">'
					appendList = appendList + '	<i class="fa fa-edit" onClick="editTarget(' + value
						.id + ')"></i>'
					appendList = appendList + '		<i class="fa fa-trash-o" onClick="deleteTarget(' +
						key + ',' + value.id + ')"></i>'
	...
	function editTarget(id) {
		...
		$.ajax({
			...
			success: function (result) {
				...
				Swal.fire({
					title: "Update Target",
					html: '<h4 style="float:left">Target:</h4><input id="target_sales" value="' +
						result.data.target +
						'" placeholder="[Renewal] Cisco Gold Partner" class="swal2-input">' +
						'<h4 style="float:left">Countable:</h4><input id="countable_target" value="' +
	...
	function showCertList() {
		...
		$(".img-preview").html(
			"<div class='iframe-cont'><iframe id='img-preview' width='566' height='376' src='{{asset('image/logo_partnership/certificate_placeholder.png')}}' frameborder='0' allowfullscreen></iframe></div>"
		)
		appendCertList = ''
		$.ajax({
			type: "GET",
				...
				$.each(result.data, function (key, value) {
					...
					if (pdf.split(".").pop() == "pdf") {
						...
						appendCertList = appendCertList +
							...
							'" class="btn btn-xs btn-info" id="btnPreview"><i class="fa fa-search"></i> zoom</button>'
					} else {
						appendCertList = appendCertList +
							'<img style="cursor: pointer;width:100%;height:200px;" src="{{asset("image/cert_partnership")}}/' +
							value.certificate + '" alt="Photo"><button value="' + value.certificate +
							'" style="margin-top:5px;margin-right:5px" class="btn btn-xs btn-info" id="btnPreview"><i class="fa fa-search"></i> zoom</button>'
					}
	...
	$('body').on('click', '#btnPreview', function () {
		...
		$(".timeline-footer").html(
			"<a class='btn btn-warning btn-flat btn-xs' style='display:none' id='btnUpdateCert' onClick='btnUpdateCert(" +
			$(this).next("small").next("small").text() +
			")'>Update</a> <a class='btn btn-danger btn-flat btn-xs' style='display:none' id='btnDeleteCert' onClick='btnDelCert(" +
	...
	function btnTarget(id_partnership) {
		...
		Swal.fire({
			title: titleStatus,
			html: '<h4 style="float:left">Target:</h4><input id="target_sales" placeholder="[Renewal] Cisco Gold Partner" class="swal2-input">' +
