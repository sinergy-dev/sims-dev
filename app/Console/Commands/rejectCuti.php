...
class rejectCuti extends Command
{
    ...
    public function handle()
    {
        ...
        //     CutiDetil::where('idtb_cuti_detail',$data_reject->idtb_cuti_detail)->where('status','NEW')->update(array('status' => 'REJECT'));

        //     $count = DB::table('tb_cuti_detail')->select('date_off')->where('tb_cuti_detail.id_cuti', $data_reject->id_cuti)->where('status', 'NEW')->count();
        //     if ($count = 0) {
        ...
        //             ->having("date_off", '>=', DB::raw('curdate()'))
        //             ->get();
        ...
        //         }else{
        //             $kirim = DB::table('users')->select('users.email')->where('id_territory',$data->id_territory)->where('id_position','MANAGER')->where('id_division',$data->id_division)->where('id_company','1')->first();
        //         }
        ...
        //     $decline_id = CutiDetil::where('id_cuti',$data->id_cuti)->where('status','NEW')->get();
        ...
        //     Mail::to($data->email)->cc($kirim->email)->send(new CutiKaryawan($name_cuti,$hari,$ardetil,$ardetil_after,'[SIMS-App] Permohonan Cuti (Rejected by Sistem)'));
