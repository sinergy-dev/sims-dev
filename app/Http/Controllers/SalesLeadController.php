...
class SalesLeadController extends Controller
{
    ...
    {
        $getSales = User::select(DB::raw('`nik` AS `id`,`name` AS `text`'))->where('id_division','SALES')->orWhereRaw("(`id_division` = 'SALES' OR `id_division` = 'BCD' )")->where('id_company','1')->where('status_karyawan','!=','dummy')->where('id_position','!=','ADMIN');
